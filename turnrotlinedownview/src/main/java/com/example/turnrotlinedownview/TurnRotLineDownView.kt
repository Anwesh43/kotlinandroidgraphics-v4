package com.example.turnrotlinedownview
...
fun Canvas.drawTurnRotLineDown(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawTRLDNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawTurnRotLineDown(scale, w, h, paint)
}

class TurnRotLineDownView(ctx : Context) : View(ctx) {
    ...
    data class TurnRotLineDown(var i : Int) {

        private var curr : TRLDNode = TRLDNode(0)
    ...
    companion object {
        fun create(activity : Activity) : TurnRotLineDownView {
            val view : TurnRotLineDownView = TurnRotLineDownView(activity)
