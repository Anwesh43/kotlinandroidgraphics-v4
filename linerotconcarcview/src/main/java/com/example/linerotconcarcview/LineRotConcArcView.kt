package com.example.linerotconcarcview
...
fun Canvas.drawLineRotConcArc(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2 + (w / 2) * dsc(3), h / 2) {
        for (j in 0..1) {
            ...
            val r : Float = size * 0.5f * (j + 1)
            drawArc(RectF(-r, -r, r, r), 0f, rot * dsc(2).divideScale(j, 2), false, paint)
        }
    }
}
...
fun Canvas.drawLRCANode(i : Int, scale : Float, paint : Paint) {
    ...
    drawLineRotConcArc(scale, w, h, paint)
}

class LineRotConcArcView(ctx : Context) : View(ctx) {
