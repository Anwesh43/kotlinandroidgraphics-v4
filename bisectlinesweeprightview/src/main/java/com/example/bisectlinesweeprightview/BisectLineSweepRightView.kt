...
class BisectLineSweepRightView(ctx : Context) : View(ctx) {
    ...
    data class BLRSNode(var i : Int = 0, val state : State = State()) {

        private var next : BLRSNode? = null
        private var prev : BLRSNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBLSRNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BLRSNode {
            var curr : BLRSNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BisectLineSweepRight(var i : Int) {

        private var curr : BLRSNode = BLRSNode(0)
