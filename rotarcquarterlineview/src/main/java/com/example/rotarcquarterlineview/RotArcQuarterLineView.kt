...
class RotArcQuarterLineView(ctx : Context) : View(ctx) {
    ...
    data class RAQLNode(var i : Int = 0, val state : State = State()) {

        private var next : RAQLNode? = null
        private var prev : RAQLNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawRAQLNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : RAQLNode {
            var curr : RAQLNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class RotArcQuarterLine(var i : Int) {

        private var curr : RAQLNode = RAQLNode(0)
