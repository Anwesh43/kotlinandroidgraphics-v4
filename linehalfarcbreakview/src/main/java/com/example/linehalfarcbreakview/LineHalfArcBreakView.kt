package com.example.linehalfarcbreakview
...
fun Canvas.drawLineHalfArcBreak(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2 + (h / 2 + size) * dsc(3)) {
        ...
        drawArc(RectF(0f, -size / 2, size, size / 2), 90f, 180f * dsc(1), false, paint)
    }
}

fun Canvas.drawLHABNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawLineHalfArcBreak(scale, w, h, paint)
}

class LineHalfArcBreakView(ctx : Context) : View(ctx) {
    ...
    data class LineHalfArcBreak(var i : Int) {

        private var curr : LHABNode = LHABNode(0)
